    ...
    public sealed partial class HealthAnalyzerWindow : DefaultWindow
    {
        ...
        public void Populate(HealthAnalyzerScannedUserMessage msg)
        {
            ...
            if (msg.IsAdvanced)
            {
                ...
                var reagText = new FormattedMessage();
                reagText.AddMarkupPermissive($"{Loc.GetString("blood-analyzer-window-entity-chems-text")}\n");
                if (msg.ChemStream != null && msg.ChemStream.Contents.Count > 0)
                {
                    foreach (var content in msg.ChemStream.Contents)
                    {
                        ...
                        if (!reagent.IsScanned)
                            continue;

                        var contentName = reagent.LocalizedName;
                        reagText.AddMarkupPermissive($"[color = {reagent.SubstanceColor.ToHex()}]{contentName}[/color]: {content.Quantity} " +
                        $"{(reagent.Overdose.HasValue && content.Quantity >= reagent.Overdose ? Loc.GetString("health-analyzer-window-reagent-overdos") : string.Empty)}\n");
                    }

                    Reagents.SetMessage(reagText, _allowedTags, Color.White);
                }
                else
                {
                    reagText.AddMarkupPermissive($"{Loc.GetString("blood-analyzer-window-entity-chems-none-text")}");
                    Reagents.SetMessage(reagText, _allowedTags, Color.White);
                }
            }
